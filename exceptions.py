...
class ExceptionAnalyzer:
    def __init__(self, csv_path="exceptions.xlsx", faiss_index_path="faiss_index", metadata_path="faiss_meta.pkl",
    ...
    def load_exceptions(self):
        ...
        try:
            if os.path.exists(self.csv_path):
                self.exceptions_df = pd.read_excel(self.csv_path)
    ...
    def create_vector_store(self):
        ...
        try:
            ...
            tfidf_matrix = normalize(self.vectorizer.fit_transform(self.exceptions_df["Issue"].fillna(" ")).toarray(),
                                     norm='l2')
    ...
    def search_exception(self, error_message):
        ...
        # **2. Fuzzy Matching**
        match = process.extractOne(error_message, self.exceptions_df["Issue"].dropna(), scorer=fuzz.ratio)
        print(f"match : {match}")
        if match:
            best_match, score, _ = match  # Unpacking three values
            print(f"best match : {best_match}")
            if score > 85:  # High confidence threshold
                result = self.exceptions_df[self.exceptions_df["Issue"] == best_match].iloc[0]
                return f"Fuzzy Match ({score}% Similarity): {result.to_dict()}"
...
def get_splunk_logs(search_url):
    error_list = ["com.test.Nullpointer"]
